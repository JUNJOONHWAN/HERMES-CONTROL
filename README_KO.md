...
**현재 공개 릴리스:** Nous Hermes Agent `0.18.0`용 [HERMES-CONTROL v0.1.9 (Alpha)](https://github.com/JUNJOONHWAN/HERMES-CONTROL/releases/tag/v0.1.9).
...
`0.1.9`는 **Telegram/CLI 우선**으로 설명하고 배포합니다. Telegram은 대화형 운영 조작면이고, CLI는 설치·정밀 조회·복구 조작면입니다.
...
> [!CAUTION]
> Kanban Web UI는 보조 조작면으로 포함됩니다. `v0.1.9` 릴리스 게이트는 모든 버튼, drag-and-drop 경로, WebSocket 복구 경로, 모바일 레이아웃을 포괄하는 브라우저·기기 전수검증을 완료했다고 주장하지 않습니다. 해당 acceptance matrix가 끝날 때까지 Telegram과 CLI를 권장 조작면으로 사용하십시오.
...
Kanban 원장은 부가 화면이 아니라 작업의 authoritative state machine입니다. 사람, AI worker, CLI, cron, dashboard가 같은 board DB를 읽고 씁니다. 브라우저 UI는 이 상태를 보여주고 조작하는 보조 표면 중 하나이며, `v0.1.9`의 권장 운영 경로는 Telegram과 CLI입니다.
...
`0.1.9`는 기존 `0.1.0`~`0.1.8` 번들을 보존합니다. 전체 patch는 검증된 자체 포함 HERMES-CONTROL `0.1.8` 번들에 일반적인 controller selector 수정분만 더해 재구성했습니다. 설치 과정은 공식 Nous `0.18.0` 기준선만 가져오며 개인용 private Hermes checkout이나 폐기한 control 저장소 접근을 요구하지 않습니다. OpenRouter strict-free와 OpenCode Zen은 compact supervisor 목록에 ID와 별칭이 보이는 컨트롤러 후보입니다. `openrouter`/`controller_openrouter`는 `controller_openrouter_free`로, `opencode`/`controller_opencode`는 `controller_opencode_free`로 해석되며 활성화에는 계속 health gate가 적용됩니다. OpenRouter health probe는 전체 무료 모델 우선순위 목록을 로컬에 보존하되 실제 API 요청에는 서버 제한에 맞춰 폴백을 최대 3개만 보내며, 안전한 HTTP 오류 상세를 운영자 진단에 남깁니다. 두 router는 일반 7개 Role Shell의 낮은 우선순위 worker 후보이기도 합니다.
...
0.1.5 배포의 기존 검증 기록은 다음과 같습니다. 0.1.9는 새 immutable bundle과 현재 GitHub Actions에서 별도로 검증합니다.
...
- [현재 패치 포함 경로](src/hermes_control/compatibility/hermes-agent-0.18.0-control-0.1.9/include-paths.txt): overlay bundle의 추출 범위
