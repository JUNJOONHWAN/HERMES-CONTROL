...
**현재 공개 릴리스:** Nous Hermes Agent `0.18.0`용 [HERMES-CONTROL v0.1.15 (Alpha)](https://github.com/JUNJOONHWAN/HERMES-CONTROL/releases/tag/v0.1.15).
...
`0.1.15`는 **Telegram/CLI 우선**으로 설명하고 배포합니다. Telegram은 대화형 운영 조작면이고, CLI는 설치·정밀 조회·복구 조작면입니다.
...
> [!CAUTION]
> Kanban Web UI는 보조 조작면으로 포함됩니다. `v0.1.15` 릴리스 게이트는 모든 버튼, drag-and-drop 경로, WebSocket 복구 경로, 모바일 레이아웃을 포괄하는 브라우저·기기 전수검증을 완료했다고 주장하지 않습니다. 해당 acceptance matrix가 끝날 때까지 Telegram과 CLI를 권장 조작면으로 사용하십시오.
...
Kanban 원장은 부가 화면이 아니라 작업의 authoritative state machine입니다. 사람, AI worker, CLI, cron, dashboard가 같은 board DB를 읽고 씁니다. 브라우저 UI는 이 상태를 보여주고 조작하는 보조 표면 중 하나이며, `v0.1.15`의 권장 운영 경로는 Telegram과 CLI입니다.
...
`0.1.15`는 모든 과거 번들을 보존하고 감사한 DGX LIVE 릴리스를 새 불변
overlay로 묶습니다. provider 독립 controller mode를 유지하면서 Role Shell
설정 시 승인한 기본임대, 추가임대에만 만드는 정규화된 Multitool 카드,
원 카드의 임대 provenance 표시, capability 소속 변경의 전체 영향 보고서를
추가합니다. 설치 과정은 계속 공식 Nous `0.18.0` 기준선만 가져오며 개인용
checkout이나 폐기한 저장소 접근을 요구하지 않습니다.
...
0.1.5 배포의 기존 검증 기록은 다음과 같습니다. 0.1.15는 새 immutable
...
- [현재 패치 포함 경로](src/hermes_control/compatibility/hermes-agent-0.18.0-control-0.1.15/include-paths.txt): overlay bundle의 추출 범위
