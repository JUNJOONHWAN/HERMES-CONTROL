...
공식 Hermes Agent 위에 역할 셸, 카드 계약, 교체 가능한 AI 어댑터, 중앙 Multitool 역량 catalog, 카드 단위 임대, Timeline Code Map, NeuralLink, 구조화 heartbeat를 더하는 독립 배포판입니다.
...
**현재 공개 릴리스:** Nous Hermes Agent `0.18.0`용 [HERMES-CONTROL v0.1.16 (Alpha)](https://github.com/JUNJOONHWAN/HERMES-CONTROL/releases/tag/v0.1.16).

## HERMES-CONTROL은 무엇인가

HERMES-CONTROL은 단순한 모델 라우터나 프롬프트 모음이 아닙니다. 운영자의 요청을 지속 가능한 프로젝트·카드 계약으로 만들고, 그 계약을 불변 Role Shell에 배정하며, 자격을 충족한 실행 어댑터를 선택하고, 구조화된 증거가 있을 때만 완료를 승인하는 거버넌스형 오케스트레이션 통제면입니다.

Controller, 정책, 실행자, 증명 계층을 분리했기 때문에 작업을 수행하는 AI를 바꾸더라도 작업 권한, 도구 범위, 완료 조건, 감사 이력은 조용히 바뀌지 않습니다.

## 이 오케스트레이션이 강력한 이유

강점은 특정 내장 모델이 모든 벤치마크에서 최고라는 검증되지 않은 주장이 아니라 구조에 있습니다.

- **권한은 고정하고 실행 계산은 교체합니다.** Role Shell이 허용 범위를 정의하고 adapter가 실제 실행자를 정의합니다. 모델을 바꿔도 운영 계약은 다시 쓰이지 않습니다.
- **실패 비용이 큰 곳에 강한 모델을 둡니다.** 일반 7개 역할은 저비용·무료 실행자를 쓸 수 있습니다. Hermes 자체 수선은 `hermes-repair`로 분리하고 `gpt-5.6-sol/high`를 기본값으로 두며, 교체 전 별도 평가와 운영자 승인을 강제합니다.
- **무료 모델 가용성을 고정값처럼 취급하지 않습니다.** OpenRouter strict-free와 OpenCode free가 현재 후보를 다시 읽고 유료·비정상 후보를 제외한 뒤 일반 역할에 strongest-first 폴백을 구성합니다.
- **Override는 fail-closed입니다.** 특정 카드·셸·전체에 강제한 adapter가 부적격이면 다른 실행자로 몰래 넘어가지 않습니다.
- **대화가 끝나도 업무가 남습니다.** 프로젝트, 카드, 관계, 승인, 실행, checkpoint, receipt가 개별 모델 세션과 분리되어 보존됩니다.
- **완료는 증거로 판정합니다.** 작업자의 “완료” 문장만으로 닫지 않고 provenance, 산출물, 테스트, Timeline node, Code Map 검증의 일치를 확인합니다.
- **모든 조작면이 한 상태를 공유합니다.** Telegram, CLI, 자동화, 보조 Web UI가 별도 업무 이력을 만들지 않고 같은 Project/Kanban state를 사용합니다.
...
운영자
  ...
  └─ Kanban Web UI (보조)
              │
              ▼
Root Controller — 제어 도구 6개, domain MCP 0개
              │ 생성·위임·조회·전환
              ▼
Project/Card Controller + durable Kanban/Event 원장
              │ atomic claim + immutable provenance
              ▼
Versioned Role Shell
              │ policy ∩ capabilities ∩ health ∩ capacity
              ▼
Binding / 명시적 Override
              │
              ▼
Eligible Executor Adapter
  ├─ OpenCode free / OpenRouter strict-free
  ├─ Hermes profile / Codex CLI / generic command
  └─ hermes-repair → 인증 게이트 → 기본 gpt-5.6-sol/high
              │
              ▼
산출물 + 테스트 → Timeline / Code Map → Receipt Gate → 최종 상태
```

Root Controller는 업무를 조정하지만 도메인 작업을 직접 수행하지 않습니다. 8개 Role Shell은 `code`, `market`, `browser-research`, `operations`, `report`, `verification`, `tool-management`, `hermes-repair`입니다. 다대다 Binding이 각 셸을 자격 있는 실행자와 연결하고, 명시적 Override가 셸 계약을 바꾸지 않은 채 카드 하나·셸 하나·전체 셸의 선택을 바꿉니다.

## 권장 조작면: Telegram과 CLI

`0.1.16`은 **Telegram/CLI 우선**으로 설명하고 배포합니다. Telegram은 대화형 운영 조작면이고, CLI는 설치·정밀 조회·복구 조작면입니다.

Telegram 요청 예시:

```text
현재 프로젝트, 카드, Role Shell, adapter와 health를 보여줘.
카드 t_12345678을 중지하고 checkpoint를 보존해. 관련 없는 자동화는 끄지 마.
카드 t_12345678의 같은 프로젝트에 연결된 계속 카드를 만들어줘.
이번 카드만 OpenRouter 무료 adapter로 실행해.
수선 후보 X를 평가하되 게이트를 통과하기 전에는 hermes-repair를 교체하지 마.
```
...
> [!CAUTION]
> Kanban Web UI는 보조 조작면으로 포함됩니다. `v0.1.16` 릴리스 게이트는 모든 버튼, drag-and-drop 경로, WebSocket 복구 경로, 모바일 레이아웃을 포괄하는 브라우저·기기 전수검증을 완료했다고 주장하지 않습니다. 해당 acceptance matrix가 끝날 때까지 Telegram과 CLI를 권장 조작면으로 사용하십시오.

그 밖의 명시적 한계: 무료 모델의 가용성과 품질은 언제든 변할 수 있고, NeuralLink는 의미적으로 관련된 과거 맥락을 놓칠 수 있으며, 유효한 Receipt는 기록된 계약 검사를 증명할 뿐 AI 판단의 보편적 정답을 보장하지 않습니다. 공개 배포판에는 개인 credential, 운영자 state, 비공개 노하우 DB가 포함되지 않습니다.

Timeline Code Map 소스는 DGX의 독립 저장소 한 곳에만 둡니다. CONTROL에는 그 저장소의 정확한 커밋에서 빌드한 checksum 고정 wheel만 포함됩니다. 런타임에서는 Multitool이 MCP catalog를 소유하고 모든 worker가 카드별 최소권한으로 임대받으며, worker profile이나 Hermes 소스 트리에 Timeline 소스를 복사하지 않습니다.
...
| 카드 실행 커널 | 기본 acceptance criteria/input lineage, atomic claim, dependency promotion, idempotent create, 같은 카드 terminal-contract 복구, circuit breaker, structured completion |
| Root Controller | controller-capable model/provider마다 동일한 고정 6개 Supervisor 도구를 붙이고, 설명 질문은 무도구로 답하되 실제 조회·변경은 fail-closed로 통제 |
...
| 멀티툴·MCP 관리 | 중앙 MCP·skill·plugin·toolset catalog의 카드별 최소권한 임대, 단순/고급/심층 검색 등급, TTL 회수, health probe, backup, rollback |
| 증거·완료 | shell/executor/binding provenance, 명시적 review lineage, PASS/REWORK/NEED_MORE_CAPABILITY 판정, 추론 등급 재발행, Timeline Receipt Gate |
...
Kanban 원장은 부가 화면이 아니라 작업의 authoritative state machine입니다. 사람, AI worker, CLI, cron, dashboard가 같은 board DB를 읽고 씁니다. 브라우저 UI는 이 상태를 보여주고 조작하는 보조 표면 중 하나이며, `v0.1.16`의 권장 운영 경로는 Telegram과 CLI입니다.
...
```text
Telegram / CLI / Cron / 보조 Kanban Web UI
                 │
                 ▼
...
`0.1.16`은 모든 과거 번들을 보존하고 감사한 DGX LIVE 릴리스를 새 불변
overlay로 묶습니다. `0.1.15`의 셸 임대 계약을 유지하면서, worker가
`kanban_complete` 또는 `kanban_block` 없이 정상 종료하면 명시적 unblock
전까지 같은 카드를 blocked로 유지하는 terminal-contract 재실행 방지
가드를 추가합니다. 설치 과정은 계속 공식 Nous `0.18.0` 기준선만 가져오며
개인용 checkout이나 폐기한 저장소 접근을 요구하지 않습니다.
...
# 실제 카드 dispatch; 필요할 때만 보조 웹 보드 실행
hermes-control run -- gateway start
hermes-control run -- dashboard
```
...
각 Role Shell의 기본 MCP 임대는 셸 최초 설정 때 승인 여부를 결정합니다.
승인된 기본임대는 원 카드의 claim transaction 안에서 즉시 붙고
`자동 기본임대`로 표시되며, 별도 카드나 모델 turn을 만들지 않습니다.
승인 범위 밖의 capability만 정확한 MCP·skill·toolset 요청으로 정규화해
별도 Multitool 의존 카드를 만듭니다. required·allowed·default capability
소속을 바꾸면 열린 카드, 활성 임대, 다른 셸, catalog owner, runtime
projection, 알림, Linux/macOS 배포면을 포함한 버전형 전체 영향 보고서를
반드시 기록합니다.
...
0.1.5 배포의 기존 검증 기록은 다음과 같습니다. 0.1.16은 새 immutable
bundle과 DGX 전체 회귀로 별도 검증하며, GitHub Actions 상태는 릴리스에
따로 보고합니다.
...
- [현재 패치 포함 경로](src/hermes_control/compatibility/hermes-agent-0.18.0-control-0.1.16/include-paths.txt): overlay bundle의 추출 범위
