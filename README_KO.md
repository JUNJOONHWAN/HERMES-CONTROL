...
[![Release v0.1.8 Alpha](https://img.shields.io/badge/release-v0.1.8_alpha-f59e0b.svg)](https://github.com/JUNJOONHWAN/HERMES-CONTROL/releases/tag/v0.1.8)
...
**현재 공개 릴리스:** Nous Hermes Agent `0.18.0`용 [HERMES-CONTROL v0.1.8 (Alpha)](https://github.com/JUNJOONHWAN/HERMES-CONTROL/releases/tag/v0.1.8).
...
`0.1.8`은 **Telegram/CLI 우선**으로 설명하고 배포합니다. Telegram은 대화형 운영 조작면이고, CLI는 설치·정밀 조회·복구 조작면입니다.
...
> [!CAUTION]
> Kanban Web UI는 보조 조작면으로 포함됩니다. `v0.1.8` 릴리스 게이트는 모든 버튼, drag-and-drop 경로, WebSocket 복구 경로, 모바일 레이아웃을 포괄하는 브라우저·기기 전수검증을 완료했다고 주장하지 않습니다. 해당 acceptance matrix가 끝날 때까지 Telegram과 CLI를 권장 조작면으로 사용하십시오.
...
Kanban 원장은 부가 화면이 아니라 작업의 authoritative state machine입니다. 사람, AI worker, CLI, cron, dashboard가 같은 board DB를 읽고 씁니다. 브라우저 UI는 이 상태를 보여주고 조작하는 보조 표면 중 하나이며, `v0.1.8`의 권장 운영 경로는 Telegram과 CLI입니다.
