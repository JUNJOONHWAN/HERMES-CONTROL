...
User / Gateway / CLI
  ...
  - setup-time default MCP rental approval
  - full impact report on capability membership changes
          |
          v
  Executor Adapter
  - OpenCode (default)
...
Role Shell의 기본 MCP 임대는 셸 최초 설정 시 승인 또는 거절한다. 승인된
기본분은 원 카드 claim과 같은 transaction에서 붙고 카드에 `자동
기본임대`로 표시되며, 별도 Multitool 카드를 만들지 않는다. 승인 기본분
밖의 capability만 정규화된 요청과 `tool-management` 의존 카드로
처리한다. required, allowed, default capability 소속을 바꿀 때는 열린
카드, 활성 임대, 영향받는 셸, 중앙 catalog owner, runtime projection,
알림과 CONTROL의 Linux/macOS 배포면을 포함한 전체 영향 보고서를 새 셸
버전과 함께 남긴다.
